    ...
    match users.filter(phone.eq(phone_input)).first::<User>(conn) {
        ...
        Err(QueryError::NotFound) => {
            diesel::insert_into(users::table)
                .values(NewUser{name: "New User", phone: phone_input})
...
pub fn new_login_token(conn: &PgConnection, for_user_id: i32) -> QueryResult<usize> {
    let new_token = new_token();

    diesel::insert_into(login_tokens::table)
        .values(NewLoginToken{user_id: for_user_id, token: new_token.clone()})
        .on_conflict(user_id)
        .do_update()
...
fn new_token() -> String {
    ...
    (0..6).fold("".to_string(), |out, _| format!("{}{}", out, rng.gen_range(0, 9).to_string()))
}
